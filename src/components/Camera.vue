<template>
  <div class="relative min-h-screen overflow-hidden">
    ...
    ></video>
    <button
      class="fixed left-1/2 bottom-0 transform -translate-x-1/2 rounded-full w-20 h-20 bg-gray-900 focus:outline-none"
      @click="onClick"
      :disabled="disabled"
    ></button>
    <button
      class="fixed left-2/3 bottom-0 flex items-center justify-center rounded-full w-20 h-20 bg-gray-900 focus:outline-none"
      @click="switchCamera"
    >
      <svg
    ...
    <img :src="imgSrc" alt="image" class="absolute right-0 top-0 w-60" />
  </div>
</template>
